...
class CitationResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val retrofit = Retrofit.Builder()
                .baseUrl("https://84a7fdccd1b9.eu.ngrok.io")
        ...
        call.enqueue(object : Callback<Citation?> {
            override fun onResponse(call: Call<Citation?>, response: Response<Citation?>) {
                ...
                if (!response.isSuccessful) {
                    Log.i("response", "fail")
                    author.setText("Code: " + response.code())
                    return
                }
                Log.i("body", response.toString())
                citation = response.body()
                Log.i("citation", citation.toString())
                author.setText(citation!!.book.auteur.prenom + ' ' + citation!!.book.auteur.nom)
                ...
                citationConnexesAdapter = CitationConnexesAdapter(applicationContext)
    ...
    fun showOtherBooks(v: View?) {
        ...
        intent.putExtra("books", citation!!.book.auteur.books as Parcelable)
